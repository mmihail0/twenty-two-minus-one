...
class GameState:
    def __init__(self):
        ...
        self.inventory_index     = 0         # which trump card the cursor is on
...
def run_enemy_turn(game):
    import trump
    trump.run_enemy_trumps(game)  # enemy may use a trump card before deciding to hit or stand
    while True:
...
def distribute_trumps(game):
    # draws give exactly 1 trump card each, normal rounds give 1 or 2
    count = 1 if game.round_result == "draw" else random.randint(1, 2)
    player_gains = [random.choice(TRUMP_POOL) for _ in range(count)]
    enemy_gains  = [random.choice(TRUMP_POOL) for _ in range(count)]
    game.player_inventory.extend(player_gains)
    game.player_inventory = game.player_inventory[:15]  # cap at 15
    game.enemy_inventory.extend(enemy_gains)
    game.enemy_inventory  = game.enemy_inventory[:15]
