...
FONT_BOOK_SM = pygame.font.SysFont("couriernew", 15)
FONT_BOOK_MD = pygame.font.SysFont("couriernew", 18, bold=True)
...
def add_ability_to_display(active_list, ability_name, now, other_list=None):
    ...
    For max cards, clears the persistent entry from both lists since max is global."""
...
def draw_inventory(surface, game):
    ...
    surface.blit(title, (WIDTH - panel_w + pad, 8))
    ...
    if not game.player_inventory:
        ...
        surface.blit(none_txt, (WIDTH - panel_w + pad, 38))
    else:
        for i, ability in enumerate(game.player_inventory):
            col = i % cols
            row = i // cols
            x = WIDTH - panel_w + pad + col * (ABILITY_W + gap)
            y = 34 + row * (ABILITY_H + gap)
            # draw sprite or fallback
            draw_ability_card(surface, ability, x, y)
            # highlight selected with a yellow border
            if i == game.inventory_index:
                pygame.draw.rect(surface, YELLOW, (x - 2, y - 2, ABILITY_W + 4, ABILITY_H + 4), 2, border_radius=3)
 
    hint = FONT_SM.render("[ENTER] use", True, GREY)
    hint2 = FONT_SM.render("[TAB] close", True, GREY)
    # show name of currently selected ability above hints
    if game.player_inventory:
        selected = game.player_inventory[game.inventory_index]
        name_txt = FONT_SM.render(selected.replace("_", " ").upper(), True, YELLOW)
        surface.blit(name_txt, (WIDTH - panel_w + pad, HEIGHT - 46))
    surface.blit(hint,  (WIDTH - panel_w + pad, HEIGHT - 30))
    surface.blit(hint2, (WIDTH - panel_w + pad, HEIGHT - 16))
