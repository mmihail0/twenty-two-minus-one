...
def draw_state_label(surface, game, player_stood, enemy_stood):
    if game.state == "player_turn":
        if player_stood:
            text, colour = "YOU STOOD — waiting for enemy...   [TAB] inventory", GREY
        else:
...
def run():
    ...
    pygame.display.set_caption("21 — RE7 Blackjack (Option 3: Alternating)")
    ...
    # each "exchange" = player acts once, then enemy acts once automatically
    # once both have stood, resolve
    player_stood       = False
    enemy_stood        = False
    enemy_timer        = 0
    enemy_acted_this_exchange = False  # enemy gets one action per exchange
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                elif game.state == "player_turn" and not player_stood:
                    if event.key == pygame.K_SPACE:
                        # player hits — enemy will respond automatically after delay
                        player_hit(game)
                        enemy_acted_this_exchange = False
                        enemy_timer = now + ENEMY_STEP_DELAY
                        game.state = "enemy_turn"

                    elif event.key in (pygame.K_LSHIFT, pygame.K_RSHIFT):
                        # player stands — enemy still gets one more action
                        player_stood = True
                        enemy_acted_this_exchange = False
                        enemy_timer = now + ENEMY_STEP_DELAY
                        game.state = "enemy_turn"
                ...
                elif game.state == "player_turn" and player_stood:
                    # player already stood, can still use inventory
                    if event.key == pygame.K_TAB:
                        game.state = "inventory"
        ...
        # enemy takes its action after the delay, then hands back to player
        if game.state == "enemy_turn" and now >= enemy_timer and not enemy_acted_this_exchange:
            # if enemy already busted before their turn, resolve immediately
            if game.enemy_total > game.current_max:
                ...
                enemy_acted_this_exchange = True

                if player_stood and enemy_stood:
                    resolve_round(game)
                else:
                    game.state = "player_turn"
        ...
        draw_state_label(screen, game, player_stood, enemy_stood)
